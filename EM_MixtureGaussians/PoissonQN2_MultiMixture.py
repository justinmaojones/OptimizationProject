...
def EM_(c,gammas0,thetas0,maxit=5,printing=False,returntype=0):
    C = inputdata(c)
    theta_prev = theta_matrix(gammas0,thetas0)*1.0
    theta_next = M_(C,theta_prev)
    
    #rg = conv(C,theta_next,g_(C,theta_next))
    rg = la.norm(dQ_(C,theta_next))
    t = 0
    
    Log = []
    line = t,theta_prev.T,rg
    Log.append(line)
    ...
    while t < maxit and rg >= 1e-6:
        ...
        printlines(line,printing)
    if returntype == 0:
        gammas,thetas = theta_list(theta_next)
        return gammas,thetas,t,rg,Log
    else:
        return theta_next
...
def QN2(c,gammas0,thetas0,maxit=100,ftol=1e-6,printing=False):
    C = inputdata(c)
    numparams = len(gammas0+thetas0)
    theta = theta_matrix(gammas0,thetas0)*1.0
    theta = EM_(c,gammas0,thetas0,maxit=5,returntype=1)
    ...
    while t < maxit and rg >= ftol:
        ...
        line = t,theta.T,alpha,foundalpha,rg
        Log.append(line)
        printlines(line,printing)
